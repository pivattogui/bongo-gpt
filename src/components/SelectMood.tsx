...
function SelectMood({ mood, setMood }: { mood: string, setMood: (mood: string) => void }) {
            ...
            <Listbox value={mood} onChange={setMood}>
                <div className="relative mt-1">
                    <Listbox.Button className="relative w-full cursor-pointer rounded-lg sm:bg-neutral-700 text-white py-2 pl-3 sm:pr-10 sm:text-left sm:shadow-md focus:outline-none focus-visible:border-neutral-500 focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-opacity-75 focus-visible:ring-offset-2 focus-visible:ring-offset-orange-300 sm:text-sm">
                        <span className="block truncate">{translateMoods(t, mood)}</span>
                ...
                <Transition
                    ...
                    leaveTo="opacity-0"
                >
                    <Listbox.Options className="mt-1 max-h-60 w-full sm:w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm">
                        {OPTIONS.map((option) => (
                            <Listbox.Option
                                key={option}
                                value={option}
