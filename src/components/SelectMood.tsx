        ...
        <div className="fixed right-1 top-1 w-36">
            <Listbox value={mood} onChange={setMood}>
                ...
                <Transition
                    ...
                    leaveTo="opacity-0"
                >
                    <Listbox.Options className="absolute mt-1 max-h-60 w-full sm:w-fit overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm">
                        {OPTIONS.map((option) => (
                            <Listbox.Option
                                key={option.mood}
                                value={option}
